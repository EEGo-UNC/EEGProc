...
def test_trace_survives_error_and_complete_marker_verifies_arrays(tmp_path):
    ...
    recorder = TrialRecorder(arm / "attempt_0002", snapshot_every=10)
    recorder.record({"step": 0}, {"z": np.ones((1, 2))})
    recorder.record({"step": 1}, {"z": np.ones((1, 2)) * 2})
    recorder.finish({"summary": {}, "arrays": {"z": np.ones((1, 2))}}, {})
    assert (recorder.directory / "trajectory/step_000001.npz").exists()
