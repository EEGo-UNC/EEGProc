#!/bin/bash
#SBATCH --job-name=v5_oracle_val
#SBATCH --output=v5_oracle_val_%j.out
#SBATCH --error=v5_oracle_val_%j.err
...
python -m src.eegproc.deep_learning.joint_architectures.joint_v5_sts.joint_sts_model_train \
    ...
    --n-channels 14 \
    --n-bands 3 \
    --out-dir runs/joint_v5_sts/DREAMER/valence/oracle \
    --run-name dreamer_valence_joint_v5_oracle \
    --n-jobs 1 \
    --outer-verbose 0 \
    --final-verbose 0 \
    --seed 42 \
