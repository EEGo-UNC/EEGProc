#!/bin/bash
#SBATCH --job-name=inverse_subject_vae_loso
#SBATCH --output=inverse_subject_vae_loso_%j.out
#SBATCH --error=inverse_subject_vae_loso_%j.err
...
python "$SUBMIT_DIR/inverse_subject_vae_train.py" \
    ...
    --n-channels 14 \
    --n-bands 3 \
    --out-dir runs/inverse_subject_vae/DREAMER \
    --run-name dreamer_inverse_subject_sts_vae \
