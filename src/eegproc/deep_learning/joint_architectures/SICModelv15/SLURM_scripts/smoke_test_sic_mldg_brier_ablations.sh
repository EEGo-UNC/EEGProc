...
# independent branch decoders with convex joint reconstruction, and a
# trial-level BiGRU classifier with no averaging across EEG windows.
...
# active encoder branch is decoded independently back to the EEG window; when
# both are active, their outputs also form the learned joint reconstruction.
MODEL_CONFIG="$(python - \
    "$use_gcn_gru" \
    "$use_bilstm" \
    "$remove_median" \
...
print(json.dumps({
    ...
    # There is no arithmetic mean across the trial window axis.
...
echo "Decoder: independent branches + learned convex joint reconstruction, reconstruction_weight=$RECONSTRUCTION_LOSS_WEIGHT dropout=$DECODER_DROPOUT"
