...
The current SIC encoder is deterministic. It does not contain an encoder VAE, a learned \(z\) projection, reparameterized sampling, or an encoder KL loss. It does contain two optional deterministic reconstruction decoders and, in v15, a learned convex joint reconstruction.
...
flowchart TD
    ...
    B --> C
    G --> DG["GCN-GRU decoder"]
    B --> DB["BiLSTM decoder"]
    DG --> J["Learned convex joint reconstruction"]
    DB --> J
...
### Independent and joint reconstruction

Each active branch has its own `GCNMTLDecoder`, and both decoder outputs match
the original EEG window shape. With both branches active, v15 learns one scalar
logit and converts it to a convex weight:

$$
\alpha=\sigma(a),\qquad
\hat X_{\text{joint}}
=\alpha\hat X_{\text{GCN-GRU}}
+(1-\alpha)\hat X_{\text{BiLSTM}}.
$$

The default source reconstruction objective is

$$
L_{\text{reconstruction}}
=\frac{L_{\text{joint}}+0.25L_{\text{branch-mean}}}{1.25}.
$$

`joint_reconstruction_auxiliary_weight` replaces `0.25` in this expression.
The independent losses prevent either branch from hiding a weak reconstruction
behind the other. For single-branch ablations, the objective remains that
branch's MSE. `joint_reconstruction_gain_vs_best_branch` is positive when the
joint MSE is lower than both branch MSEs.
...
+
w_{\text{reconstruction}}L_{\text{reconstruction}}
+
...
There is no encoder-VAE term in this objective. The reconstruction term is
active when `use_decoder=true`.
...
The branch decoders and joint scalar are frozen by default during calibration.
Setting `calibration_freeze_decoder=false` trains all three reconstruction paths
with `calibration_decoder_loss_weight`.
...
There is no `no_decoder` profile because the supplied profiles keep the decoder
enabled; use `use_decoder=false` for that ablation.
...
The deterministic reconstruction settings `use_decoder`, `decoder_dropout`,
`reconstruction_loss_weight`, `joint_reconstruction_auxiliary_weight`, and
`joint_reconstruction_initial_alpha` are supported in v15.
