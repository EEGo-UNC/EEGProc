...
def run(args):
    ...
    print(
        f"Physiological validity: VCSC, physiological_weight={loss.physiological_weight}"
        + (
            " (computed and logged every step, but not optimized)."
    ...
    for index in indices:
        ...
        for branch, details in summary["decoded_trials"].items():
            ...
            print(
                f"    VCSC {branch}: reconstructed_original={details['vcsc_original_reconstruction']:.6g} "
    ...
    aggregate = {
        ...
        #Plausibility of the generated features is the primary quantity here,
        #so report VCSC for the reconstructed original and the counterfactual
        #separately: the delta is what the counterfactual search cost, while
        #the baseline is decoder reconstruction error and would otherwise be
        #silently attributed to the counterfactual.
        "mean_vcsc_reconstructed_original": {
            name: float(
                np.mean(
                    [s["decoded_trials"][name]["vcsc_original_reconstruction"] for s in summaries]
                )
            )
            for name, _ in optimizer.branches
        },
        "mean_vcsc_counterfactual": {
            name: float(
                np.mean([s["decoded_trials"][name]["vcsc_counterfactual"] for s in summaries])
            )
            for name, _ in optimizer.branches
        },
        "mean_vcsc_delta": {
            name: float(
                np.mean([s["decoded_trials"][name]["vcsc_delta"] for s in summaries])
            )
            for name, _ in optimizer.branches
        },
        #Class-flip rate, independent of the confidence threshold. The
        #threshold is unreachable on this model (max observed confidence
        #across 7866 training predictions is 0.5136), so success alone would
        #report 0 everywhere and hide flips that did occur.
        "latent_class_flip_rate": float(
            np.mean(
                [
                    s["latent_counterfactual"]["predicted_class"] != s["original"]["predicted_class"]
                    for s in summaries
                ]
            )
        ),
        "decoded_class_flip_rate": {
            name: float(
                np.mean(
                    [
                        s["decoded_trials"][name]["counterfactual"]["predicted_class"]
