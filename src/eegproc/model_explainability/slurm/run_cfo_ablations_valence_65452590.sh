#!/bin/bash
#SBATCH --job-name=cfo_ablations_65452590
#SBATCH --output=cfo_ablations_65452590_%A_%a.out
#SBATCH --error=cfo_ablations_65452590_%A_%a.err
...
# Default: all 23 subjects, four concurrent one-GPU jobs. Single subject:
#   sbatch --array=0 src/eegproc/model_explainability/slurm/run_cfo_ablations_valence_65452590.sh
# Optional: TRIAL_IDS="8", MAX_STEPS=5, OUT_ROOT, RESUME=1.
# Local command preview (no GPU, module loads, or result writes):
#   DRY_RUN=1 VENV_DIR=venv bash src/eegproc/model_explainability/slurm/run_cfo_ablations_valence_65452590.sh
...
CONFIG_DIR="${CONFIG_DIR:-$PROJECT_DIR/runs/full/sic_trial_bigru_v11_mldg_brier_ablation/DREAMER/valence/suite_65452590/full/dreamer_valence_sic_trial_bigru_v11_mldg_full_full_20260827_213158/configuration_0001}"
...
OUT_ROOT="${OUT_ROOT:-$PROJECT_DIR/runs/counterfactuals/valence_suite_65452590_ablations_${RUN_ID}}"
DRY_RUN="${DRY_RUN:-0}"
RESUME="${RESUME:-0}"
...
print(json.dumps(dict(raw_eeg_npy=sys.argv[1], raw_labels_npy=sys.argv[2],
    dataset="dreamer", label_dimension="valence",
    model_module="eegproc.deep_learning.joint_architectures.SICModelv11.sic_model",
    fs=128, window_sec=1,
...
echo "Suite 65452590 / configuration 0001 / valence / subject $SUBJECT_ID"
...
echo "Output: $OUT_DIR"
echo "Artifacts: JSON/CSV summaries only (NPZ output disabled)"
if [[ "$DRY_RUN" == 1 ]]; then
...
required = {"include_target_latent", "include_typicality_no_physiology", "fixed_joint_alpha"}
if not required.issubset({action.dest for action in build_parser()._actions}):
    raise SystemExit("ERROR: sync the runner and results updates for the four-arm ablation with fixed reconstruction mixing before submitting")
...
if study["arguments"].get("fixed_joint_alpha") != 0.49751:
    raise SystemExit("ERROR: incorrect fixed reconstruction weight in study")
if fold.get("reconstruction", {}).get("joint_reconstruction_weight_source") != "fixed_override":
    raise SystemExit("ERROR: fixed reconstruction mixing was not recorded")
if fold["reconstruction"].get("joint_reconstruction_alpha") != 0.49751:
    raise SystemExit("ERROR: incorrect fixed reconstruction weight in fold")
