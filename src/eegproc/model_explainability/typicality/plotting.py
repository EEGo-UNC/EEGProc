...
def plot_report(report_directory, output):
    ...
    groups = ("observed_class_0", "base", "typicality", "observed_class_1")
    for task in tasks:
        ...
        ax.set(xticks=range(1, 5), xticklabels=["Observed class 0", "Base CFO", "Typicality CFO", "Observed class 1"],
               ylabel=r"$D_{C_1} - \tau_{C_1}$ (fold threshold subtracted)", title=f"{task.title()}: trial discrepancy")
        ...
        for objective, color in (("base", "#536878"), ("typicality", "#087f8c")):
            selected = sorted([r for r in subjects if r["task"] == task and r["objective"] == objective
                               and r["typical_percent"]], key=lambda r: int(r["subject_id"]))
            ax.plot([int(r["subject_id"]) for r in selected], [float(r["typical_percent"]) for r in selected],
                    "o-", color=color, label=objective)
        ax.set(xlabel="Held-out subject", ylabel="Typical trials (%)", ylim=(-3, 103), title=f"{task.title()}: per-subject typicality")
        ax.legend()
    ...
    for example in report["examples"]:
        ...
        axes[0].legend()
        axes[1].plot(steps, [float(r["discrepancy"]) for r in history])
        axes[1].axhline(example["threshold"], linestyle="--", color="grey")
        axes[1].set(ylabel=r"$D_{C_1}$")
        axes[2].plot(steps, [float(r["d_z"]) for r in history], label="VC sequence displacement")
