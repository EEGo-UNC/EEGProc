...
subject_<id>/
  ...
  trial_<id>/
    ...
    base/attempt_0001/           same structure for typicality/attempt_0001/
      ...
      trajectory/step_*.npz      current/best latent, VC sequence, decoded outputs,
                                raw gradient, classifier embedding/logits, Adam slots
...
Scalars are flushed every step. Tensor snapshots default to every step;
`--snapshot-every N` reduces storage while retaining step 0, the last finite
state, the selected endpoint, and every scalar row. Complete tensors can be
large for full DREAMER sequences; estimate storage from a short run before a
23-fold study. Files remain readable with `np.load(..., allow_pickle=False)`.
