...
def build_parser():
    ...
    parser.add_argument(
        ...
        default=0.0,
        help="Weight on the VCSC physiological penalty. At 0 the term is still "
        "computed and logged every step, but does not influence optimization.",
    )
    parser.add_argument(
        "--vcsc-distance-cm",
        type=_nonnegative_float,
        default=12.0,
        help="Volume-conduction radius d_c; the distance weight explodes below it.",
    )
    parser.add_argument(
        "--vcsc-tau-cm",
        type=_positive_float,
        default=4.0,
        help="Decay constant tau for the distance weight w(d).",
    )
    parser.add_argument(
        "--vcsc-z0",
        type=_nonnegative_float,
        default=2.0,
        help="Deviation tolerance Z0; pairs are penalized only beyond it.",
    )
    parser.add_argument(
        "--vcsc-z-max",
        type=_positive_float,
        default=20.0,
        help="Cap on per-pair deviation before exponentiation; prevents float32 "
        "overflow and stops one extreme pair dominating the gradient.",
    )
    parser.add_argument(
        "--stop-on-success",
        action="store_true",
